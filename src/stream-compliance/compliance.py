...
log_handler = logging.handlers.TimedRotatingFileHandler(
    f"/compliance/compliance_partition_{partition}",
    when="H",
    interval=1,
    encoding="utf-8",
    utc=True,
...
logger = logging.getLogger("compliance_logger_{}".format(partition))
...
def stream_connect(partition):
    response = requests.get(
        "https://api.twitter.com/labs/1/tweets/stream/compliance?partition={}".format(
            partition
        ),
        headers={
            "User-Agent": "Python igorbrigadir/covid19-twitter-stream-tool v0.0.2",
...
def main():
    ...
    while True:
        print(f"Connecting to Compliance Stream Partition: {partition}")
        try:
